...
namespace UCSBLL
{
    public abstract class BaseService<T> where T : class, new()
    {
        ...
        public JsonModel ReturnCanVisitFields(string accountNo)
        {
            JsonModel jsonModel = null;
            try
            {
                if (string.IsNullOrEmpty(accountNo))  //系统账号为空
                {
                    jsonModel = new JsonModel()
                    {
                        errNum = 3,
                ...
                //    return jsonModel;
                //}               
                DataTable entityDt=CurrentDal.ReturnCanVisitFields(CurrentEntity, accountNo);
                string rtnFields = string.Join(",",(from r in entityDt.AsEnumerable()
                                      select r["value"].ToString()).ToArray());
                if (string.IsNullOrEmpty(rtnFields)) //可访问的字段为空
                {
                    jsonModel = new JsonModel()
                    {
                        errNum = 2,
