...
function makePageBar(jsMethodName, pageContainer, pgIndex, pgCount, gpSize, roCount) {
    ...
    groupFirstPageIndex = (Math.floor(((pgIndex - 1) / 5)) * 5) + 1;
    //获得页码组总个数
    ...
    do {
        ...
        tempI++;
    } while (tempI < groupFirstPageIndex + 5 && tempI <= pgCount);//1.不能超过当前页码组最后一个下标 2.不能超过总页数
    ...
    if (groupFirstPageIndex + 5 <= pgCount) {
